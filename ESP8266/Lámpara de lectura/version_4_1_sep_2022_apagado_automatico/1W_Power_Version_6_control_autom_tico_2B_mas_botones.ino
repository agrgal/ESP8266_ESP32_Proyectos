...
void setup() {
  ...
  // Botón de apagado en 2 min
  server.on("/t2", HTTP_GET, [](AsyncWebServerRequest *request){
     if (tiempoApagado==0) {
        tiempoApagado = tiempo + 2*60*1000; // tiempo de apagado en 2 minutos
     }
     request->send(SPIFFS, "/index.html", String(), false, processor);
  });

  // Botón de apagado en 2 min
  server.on("/t20", HTTP_GET, [](AsyncWebServerRequest *request){
     if (tiempoApagado==0) {
        tiempoApagado = tiempo + 20*60*1000; // tiempo de apagado en 20 minutos
     }
     request->send(SPIFFS, "/index.html", String(), false, processor);
  });
